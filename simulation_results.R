...
sps = sp %>% filter(query_cost<180) %>%
  group_by(k,query_cost, seed_set_id) %>%
  summarise(
    n = n(),
...
# show distribution of spread sizes
ggplot(
  aes(x = query_cost, y = spread_size_percent,
      color = factor(k), group = query_cost),
  data = sp
) +
  facet_wrap( ~ k, labeller = label_both) +
  scale_x_log10() +
...
ggplot(
  aes(x = factor(query_cost), y = spread_size_percent,
      color = factor(k), group = paste(query_cost, sprintf("%02d", k))),
  data = sp
) +
  geom_boxplot(
    #outlier.shape = NA,
...
query_cost_to_node_cost_trans_func = function(x) approx(spss$query_cost, spss$node_cost_mean, xout = x)$y
spread_query_edge_to_node_cost_trans_func = function(x) approx(spss$edge_cost_mean, spss$node_cost_mean, xout = x)$y
...
ggplot(
  ...
  scale_x_continuous(
    name = "edges queried (mean %)",
    sec.axis = sec_axis(
      ~ spread_query_edge_to_node_cost_trans_func(.),
      name = "nodes queried (mean %)"
    )
  ) +
  theme(legend.position = c(0.9, 0.25))

ggsave("figures/spread_queries_spread_size_by_edges_and_nodes_queried.pdf", width = 4.5, height = 3.5)
